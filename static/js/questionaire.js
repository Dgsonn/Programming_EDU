document.addEventListener('DOMContentLoaded', function() {
    ...
    purposeCheckboxes.forEach(cb => {
        cb.addEventListener('change', () => {
            const checkedCount = document.querySelectorAll('input[name="purpose"]:checked').length;
            if (checkedCount > 2) {
                cb.checked = false;
    ...
    document.getElementById('surveyForm').addEventListener('submit', function(event) {
        ...
        for (let [key, value] of formData.entries()) {
            ...
            if (value === 'other') {
                if (key === 'job') value = document.getElementById('job_text').value.trim();
                if (key === 'purpose') value = document.getElementById('purpose_text').value.trim();
                if (key === 'goal') value = document.getElementById('goal_text').value.trim();
                if (key === 'language') value = document.getElementById('lang_text').value.trim();
                if (key === 'field') value = document.getElementById('field_text').value.trim();
        ...
        if(finalSurveyData.purpose.length > 0) finalSurveyData.purpose = finalSurveyData.purpose.join(', ');
        if(finalSurveyData.language.length > 0) finalSurveyData.language = finalSurveyData.language.join(', ');
        if(finalSurveyData.field.length > 0) finalSurveyData.field = finalSurveyData.field.join(', ');

        // ĐỐI TƯỢNG KẾT QUẢ CUỐI CÙNG SẴN SÀNG ĐỂ LƯU TRỮ
        console.log("DỮ LIỆU KHẢO SÁT THU THẬP ĐƯỢC:", finalSurveyData);
        
        /* Ví dụ cách bạn gửi cục `finalSurveyData` này lên backend sau này:
        ...
        */
        ...
        alert("Cảm ơn bạn! Hệ thống đã ghi nhận toàn bộ câu trả lời khảo sát thành công.");
    });
